...
claude-lens 是 Claude Code 的轻量 statusline，纯 Bash + jq，153 行单文件。
...
- **行2:** `████░░ PCT% of SIZE | 5h: 余量% [±pace] (Nh) | 7d: 余量% [±pace] [(Nd)] | [extra] | [$cost] | 时长`
...
- **条件显示** - Extra usage 仅 5h>=80% 时显示；7d 倒计时仅 >=70% 时显示；会话费用仅 API 用户显示

## 历史

v0.3.1。从 v0.2.1（962 行）重构为 153 行极简方案。旧版本归档在 `_archive/`。
